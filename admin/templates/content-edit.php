...
?>
			...
			<!-- Edit Header -->
			<a href="<?php echo admin_content_url($contentType, $editItem['slug'], $editItem['custom_slug'] ?? '', $editItem['category'] ?? ''); ?>" target="_blank" class="view-website-btn">
				<span class="icon">👁️</span> <?php printf(__t('view_type'), __t('type_' . $contentType)); ?>
			</a>
			...
			<div class="editor-layout">
				...
				<div class="editor-format-switcher">
					<button type="button" class="editor-format-tab <?php echo $currentFormat === 'html'     ? 'active' : ''; ?>" data-format="html"     onclick="window.EditorCommon.switchFormat('html')">WYSIWYG</button>
					<button type="button" class="editor-format-tab <?php echo $currentFormat === 'markdown' ? 'active' : ''; ?>" data-format="markdown" onclick="window.EditorCommon.switchFormat('markdown')">Markdown</button>
				...
				<!-- Content Editor -->
						...
						<div class="editor-section">
							<h3><?php _e('article_summary', 'Article Summary'); ?></h3>
				...
				<aside class="editor-sidebar">
						<button type="submit" form="content-form" class="button-primary" id="publish-btn">
							<span class="publish-btn-icon" id="publish-btn-icon">✓</span>
							<span id="publish-btn-label"><?php echo (($editItem['status'] ?? 'published') === 'scheduled') ? __t('schedule', 'Schedule') : __t('update', 'Update'); ?></span>
						</button>
						<div class="publish-actions">
						<button type="button" id="preview-btn" class="button" title="<?php _e('preview_open_tab'); ?>">
							👁 <?php _e('preview_badge'); ?>
						</button>
					</div>
					<a href="#" class="button cancel" onclick="confirmCancel(event);"><?php _e('cancel'); ?></a>

					<div class="sidebar-panel">
						<div class="autosave-container">
							<button type="button" id="save-draft-btn" class="button small" style="margin-top:0; margin-bottom:0;"><?php _e('save_draft'); ?></button>
							<?php if (isset($appSettings['autosave_enabled']) && $appSettings['autosave_enabled']): ?>
							<label class="checkbox-label" style="padding-top:10px; margin-top:0;">
						...
						<div class="form-group" style="margin-bottom: 15px;">
							...
							<input type="date" id="date" name="date" form="content-form"
								value="<?php echo $editItem['date'] ?? date('Y-m-d'); ?>">
						</div>
						<div class="form-group" style="margin-bottom: 15px;">
							<label for="publish_at">
								<span class="label-icon">🕐</span>
								<?php _e('schedule_publish', 'Schedule Publication'); ?>
							</label>
							<?php if (($editItem['status'] ?? 'published') === 'scheduled'): ?>
							<div class="badge badge-scheduled" style="margin-bottom:8px;">
								🕐 <?php _e('scheduled_for', 'Scheduled for'); ?> <?php echo htmlspecialchars($editItem['publish_at'] ?? ''); ?>
							</div>
							<?php endif; ?>
							<input type="datetime-local" id="publish_at" name="publish_at" form="content-form"
								value="<?php echo !empty($editItem['publish_at']) ? htmlspecialchars(str_replace(' ', 'T', $editItem['publish_at'])) : ''; ?>">
							<p class="help-text"><?php _e('schedule_help', 'Leave empty to publish immediately.'); ?></p>
						</div>
						<?php if (!empty($editItem['last_modified'])): ?>
						<div class="form-group" style="margin-bottom: 15px;">
							<label>
								<span class="label-icon">🕒</span>
								<?php _e('last_modified'); ?>
							</label>
							<div class="meta-value"><?php echo admin_format_date($editItem['last_modified']); ?></div>
						</div>
						<?php endif; ?>
						<div class="form-group">
							<label for="custom_slug">🔗 <?php _e('custom_url_slug'); ?></label>
							<input type="text" id="custom_slug" name="custom_slug" form="content-form"
								value="<?php echo htmlspecialchars($editItem['custom_slug'] ?? ''); ?>"
								placeholder="<?php _e('slug_autogenerate_placeholder'); ?>">
							<p class="help-text"><?php _e('slug_help'); ?></p>
						</div>
					</div>

					<!-- Categories & Tags Panel — available for all content types -->
					<?php
					// Collect all existing categories from all content types + the categories store
					$existingCategories = [];
					...
					if (isset($data['categories'])) {
						foreach ($data['categories'] as $slug => $categoryData) {
							if (isset($categoryData['name'])) {
								$existingCategories[$categoryData['name']] = true;
							}
						}
					}
					ksort($existingCategories);

					// Collect all existing tags from all content types + the tags store
					$existingTags = [];
					foreach (['article', 'project', 'page'] as $type) {
						if (isset($data[$type])) {
							foreach ($data[$type] as $item) {
					...
					if (isset($data['tags'])) {
						foreach ($data['tags'] as $slug => $tagData) {
							if (isset($tagData['name'])) {
								$existingTags[$tagData['name']] = true;
							}
						}
					}
					ksort($existingTags);
						...
						<div class="panel-content">
							...
							<div class="form-group" style="margin-bottom: 15px;">
								...
								<input type="text" id="category" name="category" form="content-form"
									...
									value="<?php echo isset($editItem['category']) ? htmlspecialchars($editItem['category']) : ''; ?>"
								...
								<button type="button" class="button browse-btn" onclick="toggleSuggestions('category-suggestions')">
									<?php _e('browse_all'); ?>
								</button>
							</div>
							<div id="category-suggestions" class="suggestions-box" style="display: none;">
								...
								<?php foreach (array_keys($existingCategories) as $category): ?>
									<span class="item-badge category-badge" data-value="<?php echo htmlspecialchars($category); ?>">
							...
							<div class="form-group">
								<label>🏷️ <?php _e('tags'); ?></label>
								<input type="text" id="tags" name="tags" form="content-form"
									placeholder="<?php _e('tags_placeholder'); ?>"
									value="<?php echo isset($editItem['tags']) && is_array($editItem['tags']) ? htmlspecialchars(implode(', ', $editItem['tags'])) : ''; ?>">
								<button type="button" class="button browse-btn" onclick="toggleSuggestions('tag-suggestions')">
									<?php _e('browse_all'); ?>
								</button>
							</div>
							<div id="tag-suggestions" class="suggestions-box" style="display: none;">
								<?php foreach (array_keys($existingTags) as $tag): ?>
									<span class="item-badge tag-badge" data-value="<?php echo htmlspecialchars($tag); ?>">
							...
							<label class="checkbox-label">
								<input type="checkbox" name="show_tags_at_bottom" form="content-form"
									<?php echo isset($editItem['show_tags_at_bottom']) && $editItem['show_tags_at_bottom'] ? 'checked' : ''; ?>>
					...
					<?php if ($contentType === 'page'):
						...
						?>
					<!-- Page Template Panel (pages only) -->
					<div class="sidebar-panel panel-collapsible">
						<h3 class="panel-header">
							<span class="toggle-icon">▶</span>
							📐 <?php _e('page_template'); ?>
						</h3>
						<div class="panel-content panel-collapsible">
							<div class="form-group">
								<label for="page_template"><?php _e('page_template_label'); ?></label>
								<select id="page_template" name="page_template" form="content-form">
									<?php foreach ($pageTemplates as $tplKey => $tplName): ?>
									<option value="<?php echo htmlspecialchars($tplKey); ?>"
										<?php echo $selectedTemplate === $tplKey ? 'selected' : ''; ?>>
										<?php echo htmlspecialchars($tplName); ?>
									</option>
									<?php endforeach; ?>
								</select>
					...
					<div class="sidebar-panel panel-collapsible">
						<h3 class="panel-header">
							<span class="toggle-icon">▼</span>
							<?php _e('display_options'); ?>
						</h3>
						<div class="panel-content">
							<div class="publish-meta"></div>
							<div class="checkbox-group">
								<label class="checkbox-label">
									<input type="checkbox" name="show_featured_image" form="content-form"
										<?php echo isset($editItem['show_featured_image']) && $editItem['show_featured_image'] ? 'checked' : ''; ?>>
									<?php _e('show_featured_image'); ?>
								</label>
								<label class="checkbox-label">
									<input type="checkbox" name="show_date" form="content-form"
										<?php echo isset($editItem['show_date']) && $editItem['show_date'] ? 'checked' : ''; ?>>
									<?php _e('show_date'); ?>
								</label>
								<label class="checkbox-label">
									<input type="checkbox" name="show_title" form="content-form"
										<?php echo isset($editItem['show_title']) && $editItem['show_title'] ? 'checked' : ''; ?>>
									<?php _e('show_title_on_page'); ?>
								</label>
								<?php if ($contentType === 'article' || $contentType === 'project'): ?>
								<label class="checkbox-label">
									<input type="checkbox" name="show_on_homepage" form="content-form"
										<?php echo isset($editItem['show_on_homepage']) && $editItem['show_on_homepage'] ? 'checked' : ''; ?>>
									<?php _e('show_on_homepage'); ?>
								</label>
								<?php endif; ?>
								<label class="checkbox-label">
									<input type="checkbox" name="show_in_menu" form="content-form" id="show_in_menu"
										<?php echo isset($editItem['show_in_menu']) && $editItem['show_in_menu'] ? 'checked' : ''; ?>>
									<?php _e('show_in_menu'); ?>
								</label>
								<div id="menu_order_field" style="<?php echo isset($editItem['show_in_menu']) && $editItem['show_in_menu'] ? 'display: block;' : 'display: none;'; ?> margin-left: 20px;">
									<label for="menu_order" style="margin-top:0;"><?php _e('menu_order'); ?></label>
									<input type="number" name="menu_order" id="menu_order" form="content-form"
										value="<?php echo isset($editItem['menu_order']) ? (int)$editItem['menu_order'] : 0; ?>"
										min="0" max="999">
									<small style="display: block; margin-top: 5px; color: #666;"><?php _e('menu_order_help'); ?></small>
								</div>
								<script>
						...
						<div class="panel-content">
							...
							<div class="featured-image-container">
								...
								<div id="featured-image-preview" style="margin-top: 10px;">
									<img src="<?php echo htmlspecialchars($featuredDisplayUrl); ?>" alt="<?php _e('featured_image'); ?>" class="featured-preview">
								...
								<input type="hidden" id="selected-image-path" name="selected_image_path" form="content-form"
									value="<?php echo htmlspecialchars($selectedImagePathValue); ?>">
							</div>
						</div>
					</div>

					<?php if (isset($appSettings['enable_seo']) && $appSettings['enable_seo']): ?>
					<!-- SEO Panel -->
					<div class="sidebar-panel panel-collapsible">
						<h3 class="panel-header">
							<span class="toggle-icon">▶</span>
						...
						<div class="panel-content panel-collapsible">
							...
							<div class="form-group">
								...
								<input type="text" id="meta_title" name="meta_title" form="content-form" maxlength="60"
									value="<?php echo htmlspecialchars($editItem['meta_title'] ?? ''); ?>">
							...
							<div class="form-group">
								...
								<input type="text" id="meta_keywords" name="meta_keywords" form="content-form"
									value="<?php echo htmlspecialchars($editItem['meta_keywords'] ?? ''); ?>">
							</div>
							<div class="form-group">
								<label for="canonical_url"><?php _e('canonical_url'); ?></label>
								<input type="url" id="canonical_url" name="canonical_url" form="content-form"
									value="<?php echo htmlspecialchars($editItem['canonical_url'] ?? ''); ?>">
							</div>
							<div class="form-group">
								<label for="schema_type"><?php _e('schema_type'); ?></label>
								<select id="schema_type" name="schema_type" form="content-form">
									<option value=""><?php _e('none'); ?></option>
									<option value="Article"     <?php echo (isset($editItem['schema_type']) && $editItem['schema_type'] === 'Article')     ? 'selected' : ''; ?>>Article</option>
									<option value="BlogPosting" <?php echo (isset($editItem['schema_type']) && $editItem['schema_type'] === 'BlogPosting') ? 'selected' : ''; ?>>Blog Posting</option>
									<option value="NewsArticle" <?php echo (isset($editItem['schema_type']) && $editItem['schema_type'] === 'NewsArticle') ? 'selected' : ''; ?>>News Article</option>
									<option value="WebPage"     <?php echo (isset($editItem['schema_type']) && $editItem['schema_type'] === 'WebPage')     ? 'selected' : ''; ?>>Web Page</option>
								</select>
							</div>
							<div class="seo-subsection">
								...
								<div class="form-group">
									...
									<input type="text" id="og_title" name="og_title" form="content-form"
										value="<?php echo htmlspecialchars($editItem['og_title'] ?? ''); ?>">
								</div>
								<div class="form-group">
								...
								<div class="form-group" style="text-align: center;">
									...
									<div id="og-image-preview" style="display: none; margin-top: 10px;">
										...
										<?php
										$ogRawPath    = $editItem['og_image'];
										$ogDisplayUrl = strpos($ogRawPath, 'files/') === 0 ? '../' . $ogRawPath : '../files/' . $ogRawPath;
										?>
										<img src="<?php echo htmlspecialchars($ogDisplayUrl); ?>" alt="OG Preview" style="max-width: 100%; border-radius: 4px; margin-bottom: 10px; border: 1px solid var(--border-light);">
							...
							<div class="preview-box">
								...
								<div class="preview-url" id="preview_url"
									...
									data-content-type="<?php echo htmlspecialchars($contentType); ?>"></div>
	...
	function confirmCancel(event) {
		...
		if (confirm('<?php _e('confirm_cancel'); ?>')) {
			window.location.href = 'index.php?type=<?php echo urlencode($contentType); ?>';
		}
	}
