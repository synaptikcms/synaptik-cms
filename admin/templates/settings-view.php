...
function _sv_image_picker(string $field, string $currentPath): void {
	...
	<div class="sip-wrapper" data-field="<?php echo htmlspecialchars($field); ?>">
		<div class="sip-preview"<?php echo $clean ? '' : ' style="display:none"'; ?>>
			...
			<button type="button" class="button danger sip-remove-btn" data-field="<?php echo htmlspecialchars($field); ?>" name="<?php _e('remove_image'); ?>">X</button>
		</div>
		<div class="sip-controls">
			...
			<button type="button" class="button sip-browse-btn" data-field="<?php echo htmlspecialchars($field); ?>">
...
?>
		...
		<div class="tabs">
			<div class="tab <?php echo $activeTab === 'general'       ? 'active' : ''; ?>" data-tab="general">⚙️ &nbsp; <?php _e('general'); ?></div>
			<div class="tab <?php echo $activeTab === 'reading'       ? 'active' : ''; ?>" data-tab="reading">📰 &nbsp; <?php _e('settings_tab_reading'); ?></div>
			<div class="tab <?php echo $activeTab === 'writing'       ? 'active' : ''; ?>" data-tab="writing">✏️ &nbsp; <?php _e('settings_tab_writing'); ?></div>
			<div class="tab <?php echo $activeTab === 'seo'           ? 'active' : ''; ?>" data-tab="seo">🔍 &nbsp; <?php _e('seo'); ?></div>
			<div class="tab <?php echo $activeTab === 'images'        ? 'active' : ''; ?>" data-tab="images">🏞️ &nbsp; <?php _e('images'); ?></div>
			<div class="tab <?php echo $activeTab === 'contact'       ? 'active' : ''; ?>" data-tab="contact">✉️ &nbsp; <?php _e('settings_tab_contact'); ?></div>
			<div class="tab <?php echo $activeTab === 'custom_fields' ? 'active' : ''; ?>" data-tab="custom_fields">🧩 &nbsp; <?php _e('cf_tab'); ?></div>
		</div>
		...
		<form method="post" action="index.php?action=settings" enctype="multipart/form-data">
			...
			<div id="general-tab" class="tab-content" <?php echo $activeTab !== 'general' ? 'style="display: none;"' : ''; ?>>
				...
				<div class="site-settings-section">
					...
					<div style="margin-top:12px;">
						<button type="submit" name="clear_cache" form="clear-cache-form" class="button danger" onclick="return confirm('<?php echo htmlspecialchars(__t('cache_clear_confirm')); ?>')">
			...
			<div id="writing-tab" class="tab-content" <?php echo $activeTab !== 'writing' ? 'style="display: none;"' : ''; ?>>
				...
				<div class="site-settings-section">
					...
					<div class="form-group">
						<label for="timezone">🕐 <?php _e('timezone_label'); ?>:</label>
			...
			<div id="custom_fields-tab" class="tab-content" <?php echo $activeTab !== 'custom_fields' ? 'style="display: none;"' : ''; ?>>
				...
				<div class="site-settings-section">
					...
					<div class="cf-fields-list" id="cf-list-<?php echo $cfType; ?>" data-type="<?php echo $cfType; ?>">
						...
						<button type="button" class="button danger cf-delete-btn" title="<?php echo htmlspecialchars(__t('cf_delete_field')); ?>">&#x2715;</button>
						</div>
						<?php endforeach; ?>
					</div>

					<button type="button" class="button btn-secondary cf-add-btn" data-type="<?php echo $cfType; ?>">
						+ <?php _e('cf_add_field'); ?>
					</button>
					</div>
				</div>
		...
		document.addEventListener('DOMContentLoaded', function() {
			...
			function cfBuildRow(type, index) {
				...
				return `<div class="cf-field-row" data-index="${index}">
					...
					<button type="button" class="button danger cf-delete-btn" title="${cfI18n.delete}">&#x2715;</button>
				</div>`;
			}
	...
	<div id="sip-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.55);z-index:9999;align-items:center;justify-content:center;">
		<div style="background:var(--bg-white);border-radius:var(--radius-md);width:760px;max-width:95vw;max-height:85vh;display:flex;flex-direction:column;overflow:hidden;box-shadow:var(--shadow-lg);">
			<div style="display:flex;justify-content:space-between;align-items:center;padding:14px 20px;border-bottom:1px solid var(--border-light);flex-shrink:0;">
	...
	.sip-preview { display:flex; align-items:center; gap:12px; padding:10px 14px; background:var(--bg-light); border-radius:var(--radius-sm); border:1px solid var(--border-light); }
	...
	.sip-file-item { cursor:pointer; border:2px solid transparent; border-radius:var(--radius-sm); overflow:hidden; background:var(--bg-light); transition:border-color .15s; }
	...
	.sip-folder-item { cursor:pointer; padding:10px 14px; background:var(--bg-light); border-radius:var(--radius-sm); border:1px solid var(--border-light); grid-column:1/-1; transition:background .15s; }
	.sip-folder-item:hover { background:var(--primary-light); }
	</style>
	</main>
