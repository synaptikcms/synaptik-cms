...
?>
			...
			<div class="content-type-selector">
				<a href="index.php?action=add&type=article" class="button <?php echo $selectedType === 'article' ? 'active' : ''; ?>"><?php _e('new_article'); ?></a>
				<a href="index.php?action=add&type=page"    class="button <?php echo $selectedType === 'page'    ? 'active' : ''; ?>"><?php _e('new_page'); ?></a>
				<a href="index.php?action=add&type=project" class="button <?php echo $selectedType === 'project' ? 'active' : ''; ?>"><?php _e('new_project'); ?></a>
			</div>
			...
			<div class="editor-layout">
				...
				<div class="editor-main">
					<form method="post" action="index.php?action=add" enctype="multipart/form-data" id="content-form">
						...
						onclick="window.EditorCommon.switchFormat('html')">
						WYSIWYG
						</button>
						<button type="button" class="editor-format-tab" data-format="markdown"
						onclick="window.EditorCommon.switchFormat('markdown')">
						Markdown
						</button>
				...
				<aside class="editor-sidebar">
					<!-- Publish Panel -->
						<button type="submit" form="content-form" class="button-primary" id="publish-btn">
							...
							<span id="publish-btn-label"><?php _e('publish'); ?></span>
						</button>
					...
					<div class="sidebar-panel">
						...
						<div class="form-group" style="margin-bottom: 15px;">
							...
							<input type="date" id="date" name="date" form="content-form"
								value="<?php echo isset($_SESSION['form_data']['date']) ? htmlspecialchars($_SESSION['form_data']['date']) : date('Y-m-d'); ?>">
						</div>
						<div class="form-group" style="margin-bottom: 15px;">
					...
					<div class="sidebar-panel panel-collapsible">
						...
						<div class="panel-content">
							...
							<?php if (empty($existingCategories)): ?>
							 <em class="empty-state"><?php _e('no_categories_yet'); ?></em>
							 <?php endif; ?>
						</div>
							<?php endif; ?>
						...
						<div class="form-group">
							...
							<div id="tag-suggestions" class="suggestions-box" style="display: none;">
								...
								<?php if (empty($existingTags)): ?>
									<em class="empty-state"><?php _e('no_tags_yet'); ?></em>
								<?php endif; ?>
							</div>
							...
							<div class="checkbox-group">
								<label class="checkbox-label">
									<input type="checkbox" name="show_featured_image" form="content-form"
										<?php echo isset($_SESSION['form_data']['show_featured_image']) && $_SESSION['form_data']['show_featured_image'] ? 'checked' : ''; ?>>
									<?php _e('show_featured_image'); ?>
								</label>
								<label class="checkbox-label">
									<input type="checkbox" name="show_date" form="content-form"
										<?php echo isset($_SESSION['form_data']['show_date']) && $_SESSION['form_data']['show_date'] ? 'checked' : ''; ?>>
									<?php _e('show_date'); ?>
								</label>
								<label class="checkbox-label">
									<input type="checkbox" name="show_title" form="content-form"
										<?php echo isset($_SESSION['form_data']['show_title']) && $_SESSION['form_data']['show_title'] ? 'checked' : ''; ?>>
									<?php _e('show_title_on_page'); ?>
								</label>
								<?php if ($selectedType === 'article' || $selectedType === 'project'): ?>
								<label class="checkbox-label">
									<input type="checkbox" name="show_on_homepage" form="content-form" checked>
					...
					<div class="sidebar-panel panel-collapsible">
						...
						<div class="panel-content">
							...
							<div class="featured-image-container">
								...
								if (strpos($restoredFeaturedPath, 'files/') === 0) {
									$restoredFeaturedPath = substr($restoredFeaturedPath, 6);
								}
								...
								<input type="hidden" id="selected-image-path" name="selected_image_path" form="content-form"
									value="<?php echo htmlspecialchars($restoredFeaturedPath); ?>">
								<div id="featured-image-preview" style="display: <?php echo $restoredFeaturedPath ? 'block' : 'none'; ?>; margin-top: 10px;">
									...
									<p style="font-size:0.7em;color:gray;margin-top:0;"><?php _e('selected'); ?>: <?php echo htmlspecialchars(basename($restoredFeaturedPath)); ?></p>
								...
								<input type="file" id="image" name="image" form="content-form" style="font-size:12px;width: 100%;">
						...
						<div class="panel-content panel-collapsible">
							...
							<div class="seo-subsection">
								...
								<div class="form-group" style="text-align: center;">
									...
									<input type="hidden" id="og_image" name="og_image" form="content-form"
										value="<?php echo htmlspecialchars($_SESSION['form_data']['og_image'] ?? ''); ?>">
		...
		(function _initFormatTabs() {
			var tabs = document.querySelectorAll('.editor-format-tab');
			...
			tabs.forEach(function(btn) {
				btn.addEventListener('click', function() {
					...
					// Re-initialize the correct editor for the chosen format
					if (window.EditorCommon) {
						var active = window.EditorCommon._registry ? window.EditorCommon._registry[fmt] : null;
						if (active && typeof active.syncContent === 'function') active.syncContent();
					}
				});
			});
		})();
