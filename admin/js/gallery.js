(function() {
	...
	function setFeaturedImage(imagePath, imageUrl, imageName) {
		...
		const previewHtml = `
			<p style="font-size:0.7em;color:gray;margin-top:0;">${t('gallery_selected_image')} ${imageName}</p>
		...
		const html = `
			<div class="named-gallery-block" data-gallery-index="${blockIdx}">
				<div class="named-gallery-header">
					...
					<button type="button" class="remove-gallery-block button small danger">✕ ${t('delete')}</button>
				</div>
				<div class="gallery-shortcode-bar">
					<code class="shortcode-display">[gallery id="${blockIdx}"]</code>
					<button type="button" class="copy-shortcode-btn button small" data-shortcode='[gallery id="${blockIdx}"]'>${t('copy')}</button>
					<button type="button" class="insert-shortcode-btn button small" data-gallery-id="${blockIdx}">${t('gallery_insert_editor')}</button>
				</div>
				<div class="named-gallery-items gallery-items" data-gallery-index="${blockIdx}"></div>
				<button type="button" class="add-named-gallery-images button" data-gallery-index="${blockIdx}">+ ${t('add_images')}</button>
			</div>`;
