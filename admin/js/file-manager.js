...
document.addEventListener('DOMContentLoaded', function() {
	...
	function confirmDeleteFolder(url, foldername) {
		...
		if (hasContents) {
			message += '<br><br><span style="color: #ff3333; font-weight: bold;">' + t('fm_delete_folder_warning') + '</span>';
	...
	if (toggleViewBtn) {
		toggleViewBtn.addEventListener('click', function() {
			...
			if (currentMode === 'grid') {
				...
				this.innerHTML = '\u25A6&ensp;' + t('fm_grid_view');
			} else {
				fileGrid.classList.remove('list-view');
				this.setAttribute('data-mode', 'grid');
				this.innerHTML = '\u2630&ensp;' + t('fm_list_view');
			}
		...
		if (fileGrid) {
			fileGrid.addEventListener('mousedown', function(e) {
				...
				if (!e.target.closest('.file-item, .folder-item')) {
					...
					selectionBox.style.border = '2px dashed #4fa75c';
					selectionBox.style.backgroundColor = 'rgba(79, 167, 92, 0.2)';
	...
	function deleteSelectedItems() {
		...
		if (hasFoldersWithContent) {
			message += '<br><br><span style="color: #ff3333; font-weight: bold;">' + t('fm_delete_folder_content_warning') + '</span>';
		}
