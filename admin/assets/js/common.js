...
document.addEventListener('click', function(e) {
	const toggleBtn = e.target.closest ? e.target.closest('.topbar-new-toggle') : null;
	if (toggleBtn) {
		e.preventDefault();
		e.stopPropagation();
		const menuId = toggleBtn.id === 'topbar-new-btn-global' ? 'topbar-new-menu-global' : 'topbar-new-menu';
		const menu = document.getElementById(menuId);
		if (!menu) return;
		const isOpen = menu.classList.contains('is-open');
		document.querySelectorAll('.topbar-new-menu.is-open').forEach(function(m) { m.classList.remove('is-open'); });
		document.querySelectorAll('.topbar-new-toggle').forEach(function(b) { b.setAttribute('aria-expanded', 'false'); });
		if (!isOpen) {
...
(function() {
	function currentTheme() {
		...
		if (t !== 'light' && t !== 'dark') {
			// Filet de sécurité : data-theme absent → on le pose maintenant
			try { t = localStorage.getItem('synaptik_theme'); } catch (e) { t = null; }
			if (t !== 'light' && t !== 'dark') {
	...
	document.addEventListener('click', function(e) {
		...
		try { localStorage.setItem('synaptik_theme', next); } catch (err) {}
	});
})();
...
function initBatchOperations() {
	...
	function showCheckboxes() {
		// Toggle the column header (lives in <thead>, not regenerated by renderList)
		document.querySelectorAll('th.batch-checkbox-cell').forEach(th => {
...
document.addEventListener('DOMContentLoaded', function() {
	...
	document.addEventListener('click', function(e) {
		...
		if ((e.target.classList.contains('delete-btn') || e.target.closest('.delete-btn'))
			&& !e.target.closest('.delete-category') && !e.target.closest('.delete-tag')) {
			const button = e.target.classList.contains('delete-btn') ? e.target : e.target.closest('.delete-btn');
			const type   = button.getAttribute('data-type');
			const index  = button.getAttribute('data-index');
			const title  = button.getAttribute('data-title');

			// Not a content-row delete button (e.g. backup delete) — let its own handler run.
