...
(function () {
  ...
  // ─── Simple-link label tooltips (Dashboard, Media — no submenu) ───────
  // Shares the same panel look as .sidebar-flyout-panel, but holds only a
  // text label (no links). Only relevant once the sidebar is collapsed.

  var simpleTooltip = null;
  var simpleTooltipTimer = null;

  function initSimpleTooltips() {
    var links = elements.sidebar.querySelectorAll('.sidebar-simple-link[data-label]');
    if (!links.length) return;

    simpleTooltip = document.createElement('div');
    simpleTooltip.className = 'sidebar-flyout-panel sidebar-label-tooltip';
    simpleTooltip.setAttribute('aria-hidden', 'true');
    document.body.appendChild(simpleTooltip);

    links.forEach(function (link) {
      link.addEventListener('mouseenter', function () {
        if (state.isMobile || state.isExpanded) return;
        clearTimeout(simpleTooltipTimer);
        simpleTooltip.textContent = link.getAttribute('data-label');

        var sidebarRect = elements.sidebar.getBoundingClientRect();
        var linkRect = link.getBoundingClientRect();
        simpleTooltip.style.left = Math.round(sidebarRect.right + 4) + 'px';
        simpleTooltip.style.top  = Math.round(linkRect.top) + 'px';

        simpleTooltip.classList.add('is-visible');
        simpleTooltip.setAttribute('aria-hidden', 'false');
      });

      link.addEventListener('mouseleave', function () {
        simpleTooltipTimer = setTimeout(function () {
          simpleTooltip.classList.remove('is-visible');
          simpleTooltip.setAttribute('aria-hidden', 'true');
        }, CONFIG.flyoutDelay);
      });
    });
