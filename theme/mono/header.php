...
<div class="site-layout">
    <aside class="site-sidebar" id="mono-sidebar" aria-label="Site navigation">
        <div class="site-title"><a href="<?php echo cleanUrl('home'); ?>"><?php echo htmlspecialchars($settings['site_title'] ?? 'My Site'); ?></a></div>
        <div class="sidebar-inner">
            <nav class="site-nav" aria-label="Main navigation">
                <?php echo renderHierarchicalMenu($settings, $data); ?>
            </nav>
        </div>
        <div class="sidebar-footer"><?php $footerText = $settings['footer_text'] ?? ''; $footerText = str_replace('{year}', date('Y'), $footerText); echo strip_tags($footerText, '<a><em><strong>'); ?></div>
    </aside>
