...
$settings = loadSettings(); ?>
        <article class="content-single">
            ...
            <?php endif ?>
            <?php
            $hasMeta = (!empty($item['date']) && !empty($item['show_date'])) || (!empty($item['tags']) && is_array($item['tags']));
            ?>
            <?php if ($hasMeta): ?>
            <div class="content-meta">
                <?php if (!empty($item['date']) && !empty($item['show_date'])): ?>
                <time datetime="<?php echo htmlspecialchars($item['date']); ?>">
                    <?php echo htmlspecialchars(format_date($item['date'])); ?>
                </time>
                <?php endif; ?>
            ...
            <?php endif; ?>
            <?php $customFieldsHtml = render_item_custom_fields($item, 'project'); ?>
            <?php if ($customFieldsHtml): ?>
            <div class="mono-custom-fields"><?php echo $customFieldsHtml; ?></div>
            <?php endif; ?>
            <?php if (!empty($item['description'])): ?>
            <div class="project-desc">
                <?php echo html_entity_decode($item['description'], ENT_QUOTES | ENT_HTML5, 'UTF-8'); ?>
            </div>
            <?php endif; ?>
